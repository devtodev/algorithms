...
char* kangaroo(int x1, int v1, int x2, int v2) {
 int aheadX = (x1 < x2)?x2:x1;
 int behindX = (x1 > x2)?x2:x1;
 int aheadV = (x1 < x2)?v2:v1;
 int behindV = (x1 > x2)?v2:v1;
 int step = -1;
    
 if ((aheadV == behindV) && (aheadX == behindV))
     return "YES";
 
 if ((behindV < aheadV) ||
     ((behindV == aheadV) && (aheadX != behindX)) )
     return "NO";
    
 step = (x1 - x2) / (v2 - v1);
    
 return ((step < 1) || ((x1 - x2) % (v2 - v1) != 0))?"NO":"YES";
}
